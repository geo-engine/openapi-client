# Rust API client for geoengine_openapi_client
...
Put the package under your project folder in a directory named `geoengine_openapi_client` and add the following to `Cargo.toml` under `[dependencies]`:

```
geoengine_openapi_client = { path = "./geoengine_openapi_client" }
```
