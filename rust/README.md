# Rust API client for geoengine-api-client
...
Put the package under your project folder in a directory named `geoengine-api-client` and add the following to `Cargo.toml` under `[dependencies]`:

```
geoengine-api-client = { path = "./geoengine-api-client" }
```
