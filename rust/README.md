# Rust API client for geoengine_api_client
...
Put the package under your project folder in a directory named `geoengine_api_client` and add the following to `Cargo.toml` under `[dependencies]`:

```
geoengine_api_client = { path = "./geoengine_api_client" }
```
